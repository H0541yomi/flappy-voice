...
PlayerRun {
  ...
  durationSec: float
  timestamp: datetime
  octaveFloorHz: float      // anchor set from first sung note, section 9
}
...
GameConfig {
  ...
  octaveWidthSemitones: int       // 12 = one octave
  anchorCaptureWindowMs: int      // stable-pitch window used to set the floor
  vocalRangeClampMinHz: float     // sanity bounds so a cough/thump can't
  vocalRangeClampMaxHz: float     // become the anchor
  recenterWindowSec: float        // rolling window for adaptive re-centering
  recenterDriftRatePerSec: float  // how fast the floor is allowed to drift
}
...
2. Pitch → height mapping with octave-wrap, anchored to the player's own
   voice rather than a fixed absolute Hz band: `height = (semitoneOffset
   mod 12) / 12`, where semitone 0 is the player's own octave floor (see
   item 3 and section 9) — no calibration screen.
3. Attract mode: on load, the character auto-lerps between the Y-positions
   ...
   is detected, that pitch sets the octave floor for the run and control
   hands off to the player at that same height (no snap/teleport, no
...
10. Adaptive octave re-centering: a rolling window of sung pitch during a
    run slowly drifts the octave floor if the player is consistently
    pushing against the top or bottom edge, so the mapping tracks natural
    drift/fatigue without ever pausing the run.
...
- **Risk:** a fixed absolute-Hz octave band excludes voices that sit
  outside it (bass vs. soprano), but a dedicated calibration screen adds
  friction before the first run.
  **Decision:** no calibration screen. Anchor the octave floor implicitly
  to the first ~`anchorCaptureWindowMs` of stable pitch captured during the
  attract-to-play handoff (already a required action — see feature 3),
  clamped to `vocalRangeClampMinHz`/`Max` so a stray cough or thump can't
  become the anchor. Pair this with adaptive re-centering during the run
  (feature 10) — a rolling pitch window that drifts the floor if the
  player consistently presses against the top or bottom edge, covering
  fatigue/drift without stopping play. Together these replace explicit
  calibration while still fitting the octave to the individual player.
...
- **Hz → height mapping:** convert Hz to a MIDI note number via
  `69 + 12 * log2(f / 440)`, subtract the run's `octaveFloor` note, take
  the result `mod 12`, normalize to 0–1, and use that as the target Y.
  The modulo operation *is* the octave-wrap.
- **Octave floor anchor:** during the attract-to-play handoff, once pitch
  has been stable (low variance) for `anchorCaptureWindowMs`, set
  `octaveFloor` to that note, clamped into
  `[vocalRangeClampMinHz, vocalRangeClampMaxHz]`. This is the entire
  "calibration" — it rides on the handoff that already has to happen for
  the game to start, so there's no separate screen or wait.
- **Adaptive re-centering:** maintain a rolling buffer of sung notes over
  `recenterWindowSec`; if the buffer's mean sits persistently near 0 or 12
  (i.e., the player is hugging an edge), nudge `octaveFloor` by up to
  `recenterDriftRatePerSec` per second toward the buffer's center. Runs
  as a slow background correction — never a visible jump.
